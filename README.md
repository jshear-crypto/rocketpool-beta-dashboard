...
Note: Currently, testing has only been done for Geth as the Eth1 node and Lighthouse as the Eth2 node. Prysm is not yet supported, but will be soon. I'm also not sure how this will work with Infura as an Eth1 node, but if you test it out let me know (not really recommended though, it might use up a lot of your requests)!
...
### Lighthouse
The dashboard uses the HTTP server on Eth2 nodes to get the information. This has only been tested for Lighthouse, but should work for Prysm as well. Go to the Prysm website to see if their arguments are the same for exposing the HTTP server.
...
Now that we've made all the necessary changes, just run `rocketpool service pause && rocketpool service start` so that they take effect. Now, go enjoy the dashboard!
